...
tk.Label(root, text="Tic Tac Toe", bg="#3c3c3c", fg="#AAB7B8", font=('Showcard Gothic', 25)).pack() 
status_label = tk.Label(root, text="à X de jouer", font=('Ariel', 15), bg='#3c3c3c', fg='#AAB7B8')
...
def play_again(): 
    ...
    for point in XO_points: 
        ...
        point.reset()
    status_label.configure(text="à X de jouer") 
    play_again_button.pack_forget() 
play_again_button = tk.Button(root, text='Encore une !',bg="#3c3c3c", fg="#AAB7B8", font=('Showcard Gothic', 15), command=play_again) 
...
class XOPoint:
    def __init__(self, x, y):
        ...
        self.y = y
        self.value = None
        self.button = tk.Button(play_area, text="", width=25, height=10, command=self.set) 
        self.button.grid(row=x, column=y)  

    def set(self): 
        global current_chr 
        if not self.value: 
            self.button.configure(text=current_chr, bg='#3c3c3c', fg='#AAB7B8')
            self.value = current_chr 
            if current_chr == "X":
                X_points.append(self)
                current_chr = "O"
                status_label.configure(text="à O de jouer")
            elif current_chr == "O":
...
def check_win(): 
    for possibility in winning_possibilities: 
        if possibility.check('X'): 
            status_label.configure(text="X a gagné!") 
            disable_game() 
            return 
        elif possibility.check('O'): 
            status_label.configure(text="O a gagné!") 
            disable_game() 
            return 
    if len(X_points) + len(O_points) == 9: 
        status_label.configure(text="Egalité !") 
        disable_game() 
